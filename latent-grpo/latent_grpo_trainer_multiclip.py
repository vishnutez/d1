...
class LatentGRPOTrainer(GRPOTrainer):
    """
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if self.args.logps_eval_mode == 'merge':
            ...
            per_token_logps = per_token_logps.squeeze(0) # (bs, num_tokens_per_diffusion_step)
        ...
        if self.beta != 0.0:
            ...
            ref_per_token_logps = inputs["ref_per_token_logps"][eval_time_step_idx] # (bs, num_tokens_per_diffusion_step)
            exact_kl = ((torch.exp(all_tokens_logps) * (all_tokens_logps - ref_all_tokens_logps)).sum(dim=-1)) # (bs, num_tokens_per_diffusion_step)
            k3_estimate_kl = (
                torch.exp(ref_per_token_logps - per_token_logps) - (ref_per_token_logps - per_token_logps) - 1
            ) # (bs, num_tokens_per_diffusion_step)
        
        else:
            print(f'beta = 0.0, so no kl term is computed', flush=True)

        # Compute the loss
        advantages = inputs["advantages_per_step"][:, eval_time_step_idx] # (bs,)
        advantages = advantages.unsqueeze(1) # (bs, 1)
        ...
        coef_1 = torch.exp(per_token_logps - old_per_token_logps) # [bs, num_tokens_per_diffusion_step]
        coef_2 = torch.clamp(coef_1, 1 - self.epsilon, 1 + self.epsilon)  # [bs, num_tokens_per_diffusion_step]
        per_token_loss1 = coef_1 * advantages # [bs, num_tokens_per_diffusion_step]
        per_token_loss2 = coef_2 * advantages # [bs, num_tokens_per_diffusion_step]
        per_token_loss = -torch.min(per_token_loss1, per_token_loss2) # [bs, num_tokens_per_diffusion_step]
        if self.beta != 0.0:
        ...
        if self.args.alpha > 0.0:  # neg_logp/entropy term
            ...
            if self.args.max_entropy_regularization == 'neg_logp':
                per_token_loss = per_token_loss - self.args.alpha * (-per_token_logps) # negative logps term in loss, so minimizing logps is equivalent to maximizing this term
            elif self.args.max_entropy_regularization == 'entropy':
                per_token_loss = per_token_loss - self.args.alpha * per_token_entropies # negative entropy term in loss, so maximizing entropy is equivalent to minimizing this term
            else:
        ...
        loss = per_token_loss.sum(dim=-1).mean() / num_tokens_per_diffusion_step # scalar
        ...
        if self.beta != 0.0:
            if self.args.use_exact_kl:
                mean_kl = exact_kl.sum(dim=-1) / num_tokens_per_diffusion_step  # sum over num_tokens_per_diffusion_step
            else:
                mean_kl = k3_estimate_kl.sum(dim=-1) / num_tokens_per_diffusion_step # sum over num_tokens_per_diffusion_step
            self._metrics[mode]["exact_kl"].append(self.accelerator.gather_for_metrics(exact_kl.sum(dim=-1) / num_tokens_per_diffusion_step).mean().item())
            self._metrics[mode]["k3_estimate_kl"].append(self.accelerator.gather_for_metrics(k3_estimate_kl.sum(dim=-1) / num_tokens_per_diffusion_step).mean().item())
            self._metrics[mode]["kl"].append(self.accelerator.gather_for_metrics(mean_kl).mean().item())
        else:
            print(f'beta = 0.0, so no kl term is logged', flush=True)
        
        is_clipped = (per_token_loss1 < per_token_loss2).float() # [bs, num_tokens_per_diffusion_step]
        clip_ratio = is_clipped.mean(dim=-1).mean()  # mean over num_tokens_per_diffusion_step and over batch
    ...
    def _get_per_token_logps_unbiased(self, model, trajectory_ids, eval_time_steps, get_all_tokens_logps=False):
        ...
        # Handle 2D case (different eval_time_steps for each batch)
            ...
            num_tokens_per_diffusion_step = self.args.max_completion_length // self.args.diffusion_steps
            per_token_logps = torch.zeros(num_steps, batch_size, num_tokens_per_diffusion_step, device=device)  # [num_steps, batch_size, num_tokens_per_diffusion_step]
            ...
            for b in range(batch_size):
                ...
                for i in range(num_steps):
                    ...
                    # Compute per-token cross-entropy losses
                    per_token_losses = F.cross_entropy(pred_logits, targets, reduction="none")  # [positions.sum() = num_tokens_per_step]

                    # Compute per-token log probabilities for this batch item at this time step
                    per_token_logps[i, b, :] = -per_token_losses # [num_tokens_per_diffusion_step]
        ...
        torch.cuda.empty_cache()
        if get_all_tokens_logps:
            return per_token_logps, all_tokens_logps  # [num_steps, batch_size, num_tokens_per_diffusion_step], [num_steps, batch_size, num_tokens_per_diffusion_step, vocab_size]
        else:
    ...
    def _generate_and_score_completions(
        ...
        num_tokens_per_diffusion_step = self.args.max_completion_length // self.args.diffusion_steps
        old_per_token_logps = torch.zeros((self.args.logps_eval_num_steps, unmasked_prob_distributions.shape[0], num_tokens_per_diffusion_step), device=device, dtype=torch.float32)
        for batch_idx in range(unmasked_prob_distributions.shape[0]):
            old_per_token_logps[:, batch_idx, :] = logps[batch_idx, eval_time_steps[batch_idx]] # (self.args.logps_eval_num_steps, batch_size, num_tokens_per_diffusion_step)
        ...
        with torch.no_grad():
            ...
            if self.beta == 0.0:
                ...
                with self.accelerator.unwrap_model(self.model).disable_adapter():
                    if self.args.logps_eval_mode == 'merge':
                        ref_per_token_logps = self._get_per_token_logps_merge(
        ...
        if self.args.lambda2 > 0.0:
            print('using lambda2, so we add the value estimates to the returns_local: returns_local = returns_local + lambda2 * q_estimates', flush=True)
            
            # schedule using gamma
            if self.args.terminate_at_last_non_eos:
                if self.args.eos_scale == 'ones':
                    scales = torch.ones((last_non_eos_steps.size(0), self.args.diffusion_steps), device=device, dtype=torch.float32) # (batch_size, diffusion_steps)
                else:
                    scales = torch.zeros((last_non_eos_steps.size(0), self.args.diffusion_steps), device=device, dtype=torch.float32) # (batch_size, diffusion_steps)
                for b in range(last_non_eos_steps.size(0)):
                    scales[b, -last_non_eos_steps[b] + 1:] = self.args.gamma ** torch.arange(1, last_non_eos_steps[b], device=device, dtype=torch.float32)
                    scales[b, -last_non_eos_steps[b]] = 1.0
                # (batch_size, diffusion_steps) [0, 0, ..., 0, 1.0, gamma^1, ..., gamma^(last_non_eos_steps-1)]
                print(f'scales (batch_size, diffusion_steps) = ({scales})', flush=True)
            else:
                scales = (self.args.gamma ** torch.arange(1, self.args.diffusion_steps, device=device, dtype=torch.float32)) # (diffusion_steps,)
                scales = torch.cat([torch.ones((1,), device=device), scales]) # (diffusion_steps,) [1.0, gamma^1, ..., gamma^(diffusion_steps-1)]
                scales = scales.unsqueeze(0) # (1, diffusion_steps)
                print(f'scales (1, diffusion_steps) = ({scales})', flush=True)

            
            ## Method 1: Normalize the scales to 1 by sum
            # scales = scales / scales.sum() # normalize to 1

            ## Method 2: Normalize the scales to 1 by cumsum
            normalizations = scales.cumsum(dim=1).flip(dims=[1]) # (batch_size, diffusion_steps) [1.0+gamma^1+...+gamma^(non_eos_steps-1), ..., 1.0+gamma^1, 1.0, 0, ..., 0]    
            normalizations = torch.maximum(normalizations, torch.ones((1, self.args.diffusion_steps), device=device))  # [1.0+gamma^1+...+gamma^(non_eos_steps-1), ..., 1.0+gamma^1, 1.0, 1.0, ..., 1.0] avoid division by zero
            scales = scales.flip(dims=[1]) # (batch_size, diffusion_steps) [gamma^(non_eos_steps-1), gamma^(non_eos_steps-2), ..., gamma^1, 1.0, 0, ..., 0]
            
            print(f'scales (batch_size, diffusion_steps) = ({scales})', flush=True)


            # take only from 1:diffusion_steps+1, because the first step is the initial state only in discounted stepwise rewards
            action_rewards_per_step = rewards_per_step[:, 1:]
            scaled_action_rewards_per_step = action_rewards_per_step * scales  # (batch_size, diffusion_steps)
            print(f'scaled_action_rewards_per_step (per_device_train_batch_size, diffusion_steps) = ({scaled_action_rewards_per_step.shape})', flush=True)
            q_estimates = scaled_action_rewards_per_step.flip(dims=[1]).cumsum(dim=1).flip(dims=[1]) / normalizations
    
            print(f'q_estimates (per_device_train_batch_size, diffusion_steps) = ({q_estimates.shape})', flush=True)

            q_estimates_selected = torch.zeros(q_estimates.shape[0], self.args.logps_eval_num_steps, device=device)
            # selected time steps
            for batch_idx in range(q_estimates.shape[0]):
                q_estimates_selected[batch_idx, :] = q_estimates[batch_idx, eval_time_steps[batch_idx]]
            print(f'q_estimates_selected (per_device_train_batch_size, logps_eval_num_steps) = ({q_estimates_selected.shape})', flush=True)
            returns_local_selected += self.args.lambda2 * q_estimates_selected
            print('using lambda2, so we add the q_estimates to the returns_local: returns_local = returns_local + lambda2 * q_estimates', flush=True)
        else:
            print('using lambda2 = 0.0, so we do not add the q_estimates to the returns_local: returns_local = returns_local', flush=True)
        ...
        # GRPO baseline
        if self.args.baseline_mode == 'init':
            mean_grouped_returns = returns_grouped[:, :, 0].mean(dim=1, keepdim=True).unsqueeze(-1)  # [num_prompts, 1, 1]  init_value
        elif self.args.baseline_mode == 'final':
            mean_grouped_returns = returns_grouped[:, :, -1].mean(dim=1, keepdim=True).unsqueeze(-1)  # [num_prompts, 1, 1]  final_value
        elif self.args.baseline_mode == 'curr':
            mean_grouped_returns = returns_grouped.mean(dim=1, keepdim=True)  # [num_prompts, 1, K]  curr_value
        else:
            raise ValueError(f'Invalid baseline mode: {self.args.baseline_mode}. Available modes: init, final, curr')
        
        print(f'using baseline mode: {self.args.baseline_mode}', flush=True)
        ...
        if self.args.returns_mode == 'sequence_return':
            advantages_per_step += self.args.lambda1 * incremental_advantages_selected
            print('using group returns, so we add the incremental advantages to the group returns: net_advantage_per_step = group_advantage + lambda1 * incremental_advantages', flush=True)
