...
class LatentGRPOTrainer(GRPOTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        loss = per_token_loss.sum(dim=-1).mean() # scalar
        ...
        if self.beta != 0.0:
            if self.args.use_exact_kl:
                mean_kl = exact_kl.sum(dim=-1)  # sum over num_tokens_per_diffusion_step
            else:
                mean_kl = k3_estimate_kl.sum(dim=-1) # sum over num_tokens_per_diffusion_step
            self._metrics[mode]["exact_kl"].append(self.accelerator.gather_for_metrics(exact_kl.sum(dim=-1)).mean().item())
            self._metrics[mode]["k3_estimate_kl"].append(self.accelerator.gather_for_metrics(k3_estimate_kl.sum(dim=-1)).mean().item())
