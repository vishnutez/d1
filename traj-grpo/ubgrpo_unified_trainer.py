...
class TrajGRPOTrainer(GRPOTrainer):
    ...
    def _generate_and_score_completions(
        ...
        def _get_low_confidence_time_steps(logps):
            """
            Select time steps with lowest confidence from log probabilities.
            
            Args:
                logps: (batch_size, diffusion_steps+1, num_tokens)
                    Log probabilities for unmasked tokens.
            
            Returns:
                eval_time_steps: (batch_size, logps_eval_num_steps)
                    Indices of time steps with lowest confidence.
            """
            ...
            for batch_idx in range(batch_size):
                uncertainty = -logps[batch_idx].min(dim=-1).values # (diffusion_steps+1,) uncertainty = - log confidence
                top_k_indices = torch.topk(
                    uncertainty,
                    k=self.args.logps_eval_num_steps,
                    dim=0
                ).indices  # (self.args.logps_eval_num_steps,)
                eval_time_steps[batch_idx] = top_k_indices
            return eval_time_steps
        ...
        if self.args.logps_eval_mode == 'unbiased':
            if self.args.logps_eval_time_steps_mode == 'random':
                ...
                # Generate different random steps for each batch element
                eval_time_steps = torch.randint(0, diffusion_steps-1, (batch_size, self.args.logps_eval_num_steps-1)).to(device)
                # Add the final time step to each batch element's eval_time_steps
                final_time_step = torch.full((batch_size, 1), diffusion_steps-1, device=device)
                eval_time_steps = torch.cat([eval_time_steps, final_time_step], dim=1)
                ...
                # Repeat eval_time_steps for each batch element to have shape (batch_size, num_steps)
                eval_time_steps = eval_time_steps.unsqueeze(0).expand(batch_size, -1)
            elif self.args.logps_eval_time_steps_mode == 'high_entropy':
                eval_time_steps = _get_high_entropy_time_steps(unmasked_prob_distributions)  # (batch_size, args.logps_eval_num_steps)          
            elif self.args.logps_eval_time_steps_mode == 'low_confidence':
                eval_time_steps = _get_low_confidence_time_steps(logps)  # (batch_size, args.logps_eval_num_steps)          
            else:
                eval_time_steps = None

            print(f'logps_eval_time_steps_mode: {self.args.logps_eval_time_steps_mode}', flush=True)
            print(f'before permutation, eval_time_steps (batch_size, args.logps_eval_num_steps) = ({eval_time_steps.shape})', flush=True)

            if self.args.permute_eval_time_steps:
                # for each batch element, randomly permute the eval_time_steps
                for batch_idx in range(eval_time_steps.shape[0]):
                    eval_time_steps[batch_idx] = eval_time_steps[batch_idx][torch.randperm(eval_time_steps.shape[1])]            

            print(f'after permutation, eval_time_steps (batch_size, args.logps_eval_num_steps) = ({eval_time_steps.shape})', flush=True)
