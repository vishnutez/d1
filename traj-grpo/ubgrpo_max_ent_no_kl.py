...
class TrajGRPOTrainer(GRPOTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if self.args.logps_eval_mode == 'merge':
            ...
            per_token_logps, per_token_entropies = self._get_per_token_logps_unbiased(
                model, trajectory_ids, eval_time_steps=eval_time_steps[:, eval_time_step_idx].unsqueeze(1), get_entropies=True
            ) # returns (1, bs) squeezed to (bs,)  input: eval_time_steps.shape: (bs, 1)
            per_token_logps = per_token_logps.squeeze(0) # (bs,)
            per_token_entropies = per_token_entropies.squeeze(0) # (bs,)
        else:
        ...
        if self.args.alpha > 0.0:
            if self.args.max_entropy_regularization == 'neg_logp':
                mean_entropy_regularization = -per_token_logps.mean()
            elif self.args.max_entropy_regularization == 'entropy':
                mean_entropy_regularization = per_token_entropies.mean()
            else:
                raise ValueError(f'Invalid max_entropy_regularization: {self.args.max_entropy_regularization}')
            self._metrics[mode][self.args.max_entropy_regularization].append(self.accelerator.gather_for_metrics(mean_entropy_regularization).mean().item())
        else:
            print(f'alpha = 0.0, so no entropy regularization term is logged', flush=True)
    ...
    def _get_per_token_logps_unbiased(self, model, trajectory_ids, eval_time_steps, get_entropies=False):
        """
        ...
        # Handle 2D case (different eval_time_steps for each batch)
            ...
            per_token_logps = torch.zeros(num_steps, batch_size, device=device)  # [num_steps, batch_size]
            if get_entropies:
                per_token_entropies = torch.zeros(num_steps, batch_size, device=device)  # [num_steps, batch_size]
            ...
            for b in range(batch_size):
                ...
                for i in range(num_steps):
                    ...
                    # Compute the mean entropy of the predicted logits
                    if get_entropies:
                        eps = 1e-10
                        mean_entropy = (-torch.sum(pred_probs * torch.log(pred_probs + eps), dim=1)).mean() # scalar
                        per_token_entropies[i, b] = mean_entropy
        ...
        torch.cuda.empty_cache()
        if get_entropies:
            return per_token_logps, per_token_entropies  # [num_steps, batch_size], [num_steps, batch_size]
        else:
    ...
    def _generate_and_score_completions(
        ...
        print(f'scales (batch_size, diffusion_steps) = ({scales})', flush=True)
        scaled_rewards_per_step = rewards_per_step * scales  # (batch_size, diffusion_steps)
        print(f'scaled_rewards_per_step (per_device_train_batch_size, diffusion_steps) = ({scaled_rewards_per_step.shape})', flush=True)
        returns_local = scaled_rewards_per_step.flip(dims=[1]).cumsum(dim=1).flip(dims=[1]) / normalizations
