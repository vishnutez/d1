    ...
    def generate(
        ...
        with torch.amp.autocast(device_type='cuda', enabled=True):
            ...
            if return_metadata:
                ...
                return trajectory, logps, unmasked_prob_distributions, greedy_completions
            else:
                return trajectory
    ...
    def _generate_and_score_completions(
        ...
        with unwrap_model_for_generation(self.model_wrapped, self.accelerator) as unwrapped_model:
            ...
            for i in range(0, prompt_ids.size(0), generation_batch_size):
                ...
                batch_trajectory_ids, batch_logps, batch_unmasked_prob_distributions, batch_greedy_completions = self.generate(
        ...
        normalizations = scales.cumsum(dim=0).flip(dims=[0]).unsqueeze(0) # (1, diffusion_steps) [1.0+gamma^1+...+gamma^(diffusion_steps-1), 1.0+gamma^1, ..., 1.0]    
        scales = scales.flip(0).unsqueeze(0) # (1, diffusion_steps) [gamma^(diffusion_steps-1)/(1.0+gamma^1+...+gamma^(diffusion_steps-1)), ..., gamma^1/(1.0+gamma^1), 1.0]
        
        print(f'scales (1, diffusion_steps) = ({scales})', flush=True)
        scaled_rewards_per_step = rewards_per_step * scales
