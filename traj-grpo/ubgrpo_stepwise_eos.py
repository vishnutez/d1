...
class TrajGRPOTrainer(GRPOTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if self.args.alpha > 0.0:  # neg_logps term
            per_token_loss = per_token_loss + self.args.alpha * per_token_logps
        ...
        if self.args.alpha > 0.0:
            mean_neg_logps = -per_token_logps.mean()
            self._metrics[mode]["neg_logps"].append(self.accelerator.gather_for_metrics(mean_neg_logps).mean().item())
        else:
            print(f'alpha = 0.0, so no neg_logps term is logged', flush=True)
